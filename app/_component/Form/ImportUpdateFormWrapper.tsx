...
const ImportUpdateFormWrapper = <TRow extends Record<string, unknown>, TInput>({
  setOpen,
  transformRow,
  previewColumns,
  mutationGQL,
  additionalVariables = {},
}: ImportUpdateFormWrapperProps<TRow, TInput>) => {
  ...
  const { execute, executing } = useMutationActions({
    mutationGQL: mutationGQL || modelGQL[model].update,
    successMessage: `Update records imported successfully`,
    successDescription: `The selected records have been updated.`,
    errorMessage: "Failed to import and update records",
    errorDescription: `An error occurred while importing and updating the records.`,
  })
  ...
  const handleToSubmit = async () => {
    ...
    payload.forEach(async (item) => {
      const itemWithoutId = Object.fromEntries(Object.entries(item as Record<string, unknown>).filter(([key]) => key !== 'id'));
      await execute({
        variables: {
...
export default ImportUpdateFormWrapper
