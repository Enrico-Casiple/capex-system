...
const handleAction = async (actionType: 'archive' | 'restore' | 'delete') => {
  ...
  try {
    switch(actionType) {
      case 'archive':
        await executeArchive({ variables: { ids: selectedIds, currentUserId: session.data?.user?.id } });
        table.setRowSelection({});
        // Wait for mutation to complete, then change active state
        // setTimeout(() => setActive(true), 200);
        break;
      case 'restore':
        await executeRestore({ variables: { ids: selectedIds, currentUserId: session.data?.user?.id } });
        table.setRowSelection({});
        // setTimeout(() => setActive(true), 200);
        break;
      case 'delete':
        await executeDelete({ variables: { ids: selectedIds, currentUserId: session.data?.user?.id } });
        table.setRowSelection({});
        break;
      default:
        break;
      ...
      <Card className="p-3 bg-muted/50">
        ...
        >
          <div className="flex justify-end gap-2">
            {
              !executingArchive && (
                <Button variant="outline" type='button' onClick={() => setIsArchiveOpen(false)}>Cancel</Button>
              )
            }
            <Button variant="default" type='button' onClick={() => handleAction('archive')} disabled={executingArchive}>
        ...
        >
          <div className="flex justify-end gap-2">
            {
              !executingRestore && (
                <Button variant="outline" type='button' onClick={() => setIsRestoreOpen(false)}>Cancel</Button>
              )
            }
            <Button variant="default" type='button' onClick={() => handleAction('restore')} disabled={executingRestore}>
        ...
        >
          <div className="flex justify-end gap-2">
            ...
            <Button variant="default" type='button' onClick={() => handleAction('delete')} disabled={executingDelete}>
