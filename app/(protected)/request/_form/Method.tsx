...
const Method = (props: MethodProps) => {
  ...
  }, [requestQuery.loading, requestQuery.data?.RequestFindUnique?.data?.id, requestQuery.data?.RequestFindUnique.data, form, session.data?.user?.id])
  ...
  const handleToSubmit = async (data: unknown) => {
    ...
    if (requestCreateInput.quotationAmount < 200000 && requestCreateInput.requestedCRF?.categoryId !== '69e71cc591da596692572f71') {
      toast.error({
    ...
    //  Check the the Total of the requested items and compare it to the requested amount in the CRF, including VAT when needed.
    //  For items where `isInclusiveVat` is false, apply VAT by multiplying base amount by (1 + vatPercentage/100).
    //  Round per-item gross and total to whole numbers (no decimals) for comparison.
    const totalRequestedItemsAmount = requestCreateInput.requestItems.reduce((total, item) => {
      const qty = item.quantity ?? 1;
      // Determine base amount: prefer explicit totalPrice; otherwise use unitPrice * quantity
      const base = (item.totalPrice ?? ((item.unitPrice ?? 0) * qty)) as number;
      const vatPercent = (item.vatPercentage ?? 12) as number;
      const multiplier = item.isInclusiveVat ? 1 : (1 + vatPercent / 100);
      const gross = base * multiplier;
      // Round per-item gross to whole number
      const roundedGross = Math.round(Number.isFinite(gross) ? gross : 0);
      return total + roundedGross;
    }, 0);
    // Round requested CRF amount as well for fair comparison
    const requestedCrfAmount = Math.round(requestCreateInput.requestedCRF?.requestedAmount ?? 0);
    if (totalRequestedItemsAmount !== requestedCrfAmount) {
      toast.error({
        message: "Requested amount mismatch",
        description: `The total amount of the requested items (${totalRequestedItemsAmount}) does not match the requested amount in the CRF (${requestedCrfAmount}). Please review your request and try again.`
      })
      return fail("REQUESTED_AMOUNT_MISMATCH", `The total amount of the requested items (${totalRequestedItemsAmount}) does not match the requested amount in the CRF (${requestedCrfAmount}). Please review your request and try again.`);
    }
