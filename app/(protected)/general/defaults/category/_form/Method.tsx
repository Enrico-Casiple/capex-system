...
import { CategoryFindUnique } from '@/lib/api/gql/Category.gql';
...
const Method = (props: MethodProps) => {
  ...
  const { form, isViewMode, executingCreate, executingUpdate, handleToSubmit, tableName } =
    useFormMethod({
      ...
      model,
      modelName,
      findUniqueGQL: CategoryFindUnique,
      defaultValues,
      findUniqueQueryKey: 'CategoryFindUnique',
      createResponseKey: 'CategoryCreate',
      updateResponseKey: 'CategoryUpdate',
      transformData: (data: any) => ({
        name: data.name ? data.name : null,
        modelNameType: data.modelNameType ? `${data.modelNameType}` : null,
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <CustomStaticSelectInput
                ...
                label="Model Name"
                options={MODEL_NAMES.map((name) => ({ label: name, value: `${name}Category` }))}
                disabled={isViewMode}
              />
