...
import { JobLevelFindUnique } from '@/lib/api/gql/JobLevel.gql';
...
const Method = (props: MethodProps) => {
  ...
  const { form, isViewMode, executingCreate, executingUpdate, handleToSubmit, tableName } =
    useFormMethod({
      ...
      model,
      modelName,
      findUniqueGQL: JobLevelFindUnique,
      defaultValues,
      findUniqueQueryKey: 'JobLevelFindUnique',
      createResponseKey: 'JobLevelCreate',
      updateResponseKey: 'JobLevelUpdate',
      transformData: (data: any) => ({
        name: data.name ? data.name : null,
        acronym: data.acronym ? data.acronym : null,
        description: data.description ? data.description : null,
        companyId: data.companyId ? data.companyId : null,
        jobcodeTypeId: data.jobcodeTypeId ? data.jobcodeTypeId : null,
      }),
    });
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <CustomSingleSelectInput
                ...
                findAllWithCursorGQL={modelGQL.CompanyGQL.findAllWithCursor}
                findUniqueGQL={modelGQL.CompanyGQL.findUnique}
                defaultValueId={form.watch('jobcodeTypeId') ?? undefined}
                disabled={isViewMode}
                cursorVariables={(search, cursor, take) => ({
                  cursorInput: {
                    cursor,
