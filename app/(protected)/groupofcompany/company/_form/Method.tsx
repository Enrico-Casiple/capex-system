...
import { BasicInformation, BasicInformationResponse, Company, CompanyResponse, GroupOfCompany, GroupOfCompanyResponse, Permission, PermissionResponse } from '@/lib/generated/api/customHookAPI/graphql';
...
import { CompanyFindUnique } from '@/lib/api/gql/Company.gql';
...
const Method = (props: MethodProps) => {
  ...
  const companyFindUnique = useQuery<
    { CompanyFindUnique: CompanyResponse },
    { id: string }
  >(CompanyFindUnique, {
    variables: {
      id: props.rowId ?? "",
  ...
  useEffect(() => {
    if (props.actionType !== "none" && companyFindUnique.data?.CompanyFindUnique.data) {
      const data = companyFindUnique.data.CompanyFindUnique.data;
      form.reset({
        id: data.id,
        name: data.name ?? "",
        description: data.description ?? "",
        acronym: data.acronym ?? "",
        groupOfCompanyId: data.groupOfCompanyId ?? ""
      });
    }
  }, [companyFindUnique.data, props.actionType, form]);
  ...
  const handleToSubmit = async (data: unknown) => {
    const modelData = data as unknown as Company;
    ...
    const payload = {
      ...
      data: {
        ...
        groupOfCompanyId: modelData.groupOfCompanyId ? String(modelData.groupOfCompanyId) : null,
      },
      ...
      case 'edit':
        await executeUpdate({
          ...
          onCompleted: (data) => {
            ...
            if (!result.CompanyUpdate.isSuccess) {
              toast.error({
                message: result.CompanyUpdate.code || `Failed to update ${tableName}`,
                description: result.CompanyUpdate.message || `There was an error updating the ${tableName}. Please try again.`,
              })
              return
            }
      ...
      case "duplicate":
        await executeCreate({
          ...
          onCompleted: (data) => {
            ...
            if (!result.CompanyCreate.isSuccess) {
              toast.error({
                message: result.CompanyCreate.code || `Failed to update ${tableName}`,
                description: result.CompanyCreate.message || `There was an error updating the ${tableName}. Please try again.`,
              })
              return
            }
